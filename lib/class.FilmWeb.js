...
function createBrowser ()
{
    ...
    browser.goToUrl = (url) => {
        return browser.getCurrentUrl()
            ...
            .then(() => {
                // TODO Click on #didomi-notice-agree-button IF EXISTS
                // browser.click = (selector, timeout, failMessage) => {
                return browser
                    .click(
                        '#didomi-notice-agree-button',
                        200,
                        'FIXME Clicking on a GDPR button failed, but maybe it’s not there — then it’s ok.'
                    )
                    .catch(() => { return; });
            });
    };
    ...
    browser.sendKeys = (selector, value, timeout, failMessage) => {
        // Reluctant to log the value itself as it may be a password
        log(3, 'Sending keys to', selector, ':', JSON.stringify(value));
        return browser.wait(webdriver.until.elementLocated(webdriver.By.css(selector)), timeout, failMessage)
            .then(element => {
...
function searchDataCollector (context)
{
    ...
    {
        results.nav = query(context, '#searchPaginator .config').textContent;

        resultElements = context.querySelectorAll('.resultsList > [id^=film_]');
        Array.prototype.forEach.call(resultElements, function (listItem) {
            ...
            Array.prototype.forEach.call(subList.children, function (subItem) {
                var people;
                if (/reżyser/i.test(subItem.textContent))
                {
                    subList.activeContainer = result.directors;
                }
                else if (/obsada/i.test(subItem.textContent))
                {
                    subList.activeContainer = result.cast;
                }
                else if (subList.activeContainer)
                {
                    people = subItem.querySelectorAll('a');
...
function rate (url, value, review)
{
    ...
    let queueItem = createQueueItem();

    return queueItem.waitForItsTurn
        .then(() => bootstrap.call(this, url))
        ...
        .then(queueItem.resolved, queueItem.rejected);
}
...
class FilmWeb
{
    ...
    search (query, page=1)
    {
        ...
        return queueItem.waitForItsTurn
            .then(() => bootstrap.call(this, url))
            .then(() => browser.waitFor('.resultsList, .searchNoResults'))
    ...
    authenticate (login, password)
    {
        ...
        return queueItem.waitForItsTurn
            .then(() => bootstrap.call(this, AUTH_URL))
            .then(simulateHumanDelay)
            .then(() => browser.click('.authButton--filmweb'))
            .then(() => Promise.all([
                browser.fillUp('[name=j_username]', login),
                browser.fillUp('[name=j_password]', password),
            ]))
            .then(() => browser.sendKeys('[name=j_password]', webdriver.Key.ENTER))
